...
namespace roverControl
{
    public partial class ROVER: Form
    {
        ...
        private volatile int impulseCountForward;
        private volatile int impulseCountRight;
        ...
        private readonly double DEG_TO_RAD;
        private readonly double speed_to_mms_const;
        ...
        private Boolean invertImpulseCommandFlag; //aggiunge una 'I' al buffer (quando si tiene la direzione all'indietro)


        //variabili per prova velocità
        private int counter1sec;
        private Boolean test1sec;
        ...
        public ROVER()
        {
            ...
            this.speed_to_mms_const = 68.5; //53.75 prima

            this.speedTo_curveRadiusX[1] = 530;//millimetri
            this.speedTo_curveRadiusY[1] = 530;//690; //millimetri

            this.speedTo_curveRadiusX[2] = 525;//millimetri
            this.speedTo_curveRadiusY[2] = 525;//675; //millimetri

            this.speedTo_curveRadiusX[3] = 524; //millimetri
            this.speedTo_curveRadiusY[3] = 524;//681; //millimetri

            this.speedTo_curveRadiusX[4] = 550; //millimetri
            this.speedTo_curveRadiusY[4] = 593;//710; //millimetri
        ...
        private void commandTimer_Tick(object sender, EventArgs e)
        {
            ...
            if(comOutBuf != null && serialPort.IsOpen)
            {
                ...
                else if (this.comOutBuf[0] == 'X') this.direction = Direction.none;
            }
            ...
            this.dyTextBox.Text = this.total_dy.ToString();
            this.dxTextBox.Text = this.total_dx.ToString();
            ...
            else if(this.direction != Direction.none)
            {
                ...
                {
                    total_dy += this.speed_to_mms_const / 40 * this.roverSpeed * Math.Cos(diffAngle * DEG_TO_RAD);
                    total_dx += this.speed_to_mms_const / 40 * this.roverSpeed * Math.Sin(diffAngle * DEG_TO_RAD);
                }
                ...
                {
                    total_dy -= this.speed_to_mms_const / 40 * this.roverSpeed * Math.Sin(diffAngle * DEG_TO_RAD);
                    total_dx += this.speed_to_mms_const / 40 * this.roverSpeed * Math.Cos(diffAngle * DEG_TO_RAD);   
                }
        ...
        private void preciseTurn(int direction, int linearSpeed, double angularRate, double startTurnAngle, int destinationY, int destinationX)
        {
            ...
            double diffAngle = this.startAngleGoBack - startTurnAngle;
            ...
            int tmpdistX = distXTurn, tmpdistY = distYTurn;

            distXTurn = (int)(tmpdistX * Math.Cos(diffAngle * DEG_TO_RAD) - tmpdistY * Math.Sin(diffAngle * DEG_TO_RAD));
            distYTurn = (int)(tmpdistX * Math.Sin(diffAngle * DEG_TO_RAD) + tmpdistY * Math.Cos(diffAngle * DEG_TO_RAD));

            this.total_dx += distXTurn;
            this.total_dy += distYTurn;
        }
        ...
        private void UpdateDistanceEndTurn()
        {
            ...
            double diffAngle = this.startAngleGoBack - this.beforeTurnAngle;
            ...
            */
            ...
            double tmpdistX = dx, tmpdistY = dy;

            dx = (int)(tmpdistX * Math.Cos(diffAngle * DEG_TO_RAD) - tmpdistY * Math.Sin(diffAngle * DEG_TO_RAD));
            dy = (int)(tmpdistX * Math.Sin(diffAngle * DEG_TO_RAD) + tmpdistY * Math.Cos(diffAngle * DEG_TO_RAD));
        ...
        {
            ////this.direction = Direction.turning; messo in preciseTurn
            ...
            /* calcolo della distanza dopo 2 secondi 
            move_for_seconds(80);
            */
        }

        private void move_for_seconds(int time_25ms)
        {
            this.counter1sec = 0;
            this.test1sec = true;
            this.comOutBuf = "F5";
            while (counter1sec < time_25ms)
            {
                Application.DoEvents();
            }
        ...
        private void timerCS_Tick(object sender, EventArgs e)
        {
            if (testRunning)
            {
                ...
                if (invertImpulseCommandFlag)
                {
                    comOutBuf = comOutBuf.Insert(comOutBuf.Length, "II");
                }
            }
        }
        ...
        {
            double time;
            double mm_ms = speed * this.speed_to_mms_const / 1000;
            time = distMM/mm_ms;
        ...
        private async void goBackFastButton_Click(object sender, EventArgs e)
        {
            ...
            invertOrientation_goBack(finalAngle);
            /*
            ...
            */
            ...
            this.testRunning = false;
            //resetButton_Click(null,null);  ////////////////////////////////////////////togliere per debug
            this.comOutBuf = "X";        ////////////////////////////////////////////mettere per debug
        }

        private async void goBackImpulseButton_Click(object sender, EventArgs e)
        {
            double finalAngle = this.startAngleGoBack - 180;

            invertOrientation_goBack(finalAngle);

            this.invertImpulseCommandFlag = true;
            this.startTargetAngle = finalAngle;
            this.testRunning = true;

            if(this.impulseCountRight > 0)
            {
                this.comOutBuf = "R" + this.roverSpeed + "II";
            }
            else if (this.impulseCountRight < 0)
            {
                this.comOutBuf = "L" + this.roverSpeed + "II";
            }
            while (Math.Abs(this.impulseCountRight) > 25)
            {
                if (Math.Abs(this.impulseCountRight) < 700)
                    this.comOutBuf = this.comOutBuf.Substring(0, 1) + '1' + this.comOutBuf.Substring(2);
                await Task.Delay(1);
            }
            this.direction = Direction.none;
            this.comOutBuf = "X";

            if (this.impulseCountForward > 0)
            {
                this.comOutBuf = "F" + this.roverSpeed + "II";
            }
            else if (this.impulseCountForward < 0)
            {
                this.comOutBuf = "B" + this.roverSpeed + "II";
            }
            while (Math.Abs(this.impulseCountForward) > 25)
            {
                if (Math.Abs(this.impulseCountForward) < 700)
                    this.comOutBuf = this.comOutBuf.Substring(0, 1) + '1' + this.comOutBuf.Substring(2);
                await Task.Delay(1);
            }
            /*
            this.direction = Direction.none;
            this.comOutBuf = "X";
            this.invertImpulseCommandFlag = false;
            this.testRunning = false;
            */
        ...
        private void invertOrientation_goBack(double finalAngle)
        {
            if (finalAngle > this.currentAngle)
            {
                this.comOutBuf = "C" + this.roverSpeed;
